...
function GrabaEntrega() {
    ...
    var parametros = { "NoPrescripcion": opNoPrescripcion, "NoCuenta": opNoCuenta, "NoHistoria": opNoHistoria, "NoIdentificacion": opNoIdentificacion, "NitEntidad": opNitEntidad, "Responsable": opResponsable, "CodEPS": opCodEPS, "CodigoCum": opCUM, "NoId": NoID, "IdEntrega": IDEntrega, "CantEnt": opCantidad, "PrecioUni": opPrecio, "ValEntrega": Valor, "CodigoArt": opCodigo };
...
function GetConsInternoFact() {

    var UriRuta = '@Url.Action("MIPRESController","GetConseIntFact")'; //"lovEPS"; //'@Url.Action("controller","action")'
    var NumFact = $("#hfFactEPS").val();
 
        $.ajax({
            type: "POST", //GET
            url: "GetConseIntFact", //UriRuta,
            content: "application/json; charset=utf-8",
            dataType: "json",
            // De la siguiente manera indicamos que del div tome los input.
            data: { NoFact: NumFact }, //JSON.stringify(parametros),
            success: function (data) {
                ...
                if (msg !== "") {
                    ...
                    var res = JSON.parse(data);
                    $.each(res, function (i, item) {
                        var ConsIntFact = item["ConsInterno"];
                        $("#hfConsIntFact").val(ConsIntFact);
...
function RegFacturacion() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (Id != undefined) {
                Id = $("#hfNoID").val();
                IdEntrega = $("#hfIDEntrega").val();

                ValidaPrescripcion(opNoPrescripcion);
                
                if (validaNumPrescrip == 0) {
                    GrabaEntrega();
                }
...
function GrabaFactura(Id_F, IdFacturacion) {
    ...
    var VlrTotFact = $("#ValorTotFacturado").val();
    var ConsInFact = $("#hfConsIntFact").val(); 

    $("#hfCodEPS").val(codigoEPS);

    $("#hfVlrEntrega").val(VlrTotFact);


    var parametros = { "NoPrescripcion": NoPrescripcion, "IdEntrega": parseInt(NoIDEntrega), "NoFactura": numFactura, "CodEPS": codigoEPS, "CantEnt": parseInt(cantidadEnt), "ValorUni": parseInt(valorUni), "ID_F": parseInt(Id_F), "IdFacturacion": parseInt(IdFacturacion), "ConsIntFac": ConsInFact};
    //$("#hfIDReporteEntrega").val(IdEntrega);
...
function ValidaPrescripcion(NoPrescripcion) {

    var parametros = { "NoPrescripcion": NoPrescripcion};
    var valida = false;
    var UriRuta = "ValidarEntrega";
    $.ajax({
        type: 'POST',
        url: "ValidarEntrega",
        data: JSON.stringify(parametros), //{ "NoPrescripcion": opNoPrescripcion, "NoIdentificacion": opNoIdentificacion, "CodigoCum": opCUM, "NoId": NoID, "IdEntrega": IDEntrega, "ValEntrega": Valor },
        ...
        success: function (data) {
            ...
            var NoPrescrip = "";
            $.each(res, function (i, item) {
              NoPrescrip = item["NoPrescripcion"];
            });
            //alert("Prescripcion:" + NoPrescrip);
            if (parseInt(NoPrescrip) > 1 || NoPrescrip.length > 4) { validaNumPrescrip = 1; } 
            else { validaNumPrescrip = 0; } 
            //alert("Valida.." + validaNumPrescrip);
        },
        error: function (jqXHR, status, error) {
            alert("ERROR: " + status + " " + error + " " + jqXHR.status + " " + jqXHR.statusText)
        }
